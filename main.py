...
def update_graph(azimuth_angle, elevation_angle, distance):
    ...
    ax.scatter(x_data, y_data, z_data, c='r', marker='o', s=5)
    plt.draw()
    plt.pause(0.5)
...
async def notification_handler(characteristic: BleakGATTCharacteristic, data: bytearray):
    ...
    update_graph(degs_plus_flag.horizontal_deg, distance, degs_plus_flag.vertical_deg)
    degs_plus_flag.horizontal_deg = degs_plus_flag.horizontal_deg + 1
    if(degs_plus_flag.horizontal_deg == 180):
        degs_plus_flag.horizontal_deg = 0
        degs_plus_flag.vertical_deg = degs_plus_flag.vertical_deg + 1
        if(degs_plus_flag.vertical_deg == 45):
